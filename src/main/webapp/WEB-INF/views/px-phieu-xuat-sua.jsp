	...
	<div class="container">
		...
		<div class="title-7">
			...
			<form:form method="post" id="form" modelAttribute="formSua"
				action="${pageContext.request.contextPath}/xacnhansuaphieuxuat">
				<!-- 				 1 -->
				<div class='row'>
					<div class='col-sm-4'>
						<div class='form-group'>
							<label for="user_title">Số phiếu</label> <input
								placeholder="Mã phiếu nhập có dạng X2404171"
								class="form-control" id="user_title" name="maPhieuXuat"
								value="${layMaPhieuXuat.maPhieuXuat}" size="30" type="text" />
						</div>
					</div>
					<div class='col-sm-8'>
						<div class='form-group'>
							<label for="user_lastname">Họ tên người xuất thiết bị</label> <select
								...
								<option value="${layMaPhieuXuat.nguoiXuat}">${layMaPhieuXuat.nguoiXuat }</option>
								<c:forEach var="danhsachquantrivien"
									...
									<option value="${danhsachquantrivien.hoTen }">${danhsachquantrivien.hoTen}
										- ${danhsachquantrivien.chucVu}</option>
								</c:forEach>
							</select>
						</div>
					</div>
					...
					<div class='col-sm-8'>
						<div class='form-group'>
							<label for="user_firstname">Xuất về đơn vị (Phòng thực
								...
								name="tbNhanVien.tbPhongThucHanh.maPth" required="true" type="text">
								<option
									value="${layMaPhieuXuat.tbNhanVien.tbPhongThucHanh.maPth}">${layMaPhieuXuat.tbNhanVien.tbPhongThucHanh.phongThucHanh }</option>
					...
					<div class='col-sm-4'>
						<div class='form-group'>
							<label for="user_title">Xuất từ kho</label> <select
								...
								required="true" type="text">
								<option
									value="${layMaPhieuXuat.tbKho.maKho}">${layMaPhieuXuat.tbKho.tenKho }</option>
					...
					<div class='col-sm-8'>
						<div class='form-group'>
							<label for="user_lastname">Họ tên người nhận thiết bị</label> <select
								...
								required="true" type="text">
							<option
									value="${layMaPhieuXuat.tbNhanVien.maNql}">${layMaPhieuXuat.tbNhanVien.hoTen }</option>
								<c:forEach var="danhsachnguoidung" items="${danhsachnguoidung}">
									<option value="${danhsachnguoidung.maNql }">${danhsachnguoidung.hoTen } - ${danhsachnguoidung.tbDonVi.tenKhoa }</option>
								</c:forEach>
							</select>
						</div>
					</div>
